...
$(function () {
    ...
    $(".first_dt").css("background-image", "url(../../static/img/MinusSign.png)");
    /**一级菜单项单击事件**/
    $(".leftsidebar_box dt").click(function () {
        //判断当前一级菜单下的二级菜单项是否隐藏
        if ($(this).parent().find('dd').is(":hidden")) {
            $(this).parent().find('dd').slideToggle(); //滑动方式展开子菜单                              
            $(this).css("background-image", "url(../../static/img/MinusSign.png)");
        }
        else {
            $(this).parent().find('dd').slideUp(); //滑动方式隐藏子菜单                
            $(this).css("background-image", "url(../../static/img/AddSign.png)");
        }
    });
